<?php 
	...
	if($opt=='a'||$opt=='u'||$opt=='d'){ //add, update, delete
		...
		if($jtrans==JT_INCOME) 
			$fform->idata='transaksi pemasukan';
		else if($jtrans==JT_OUTCOME) 
			$fform->idata='transaksi pengeluaran';
		else 
			$fform->idata='jurnal umum';//jt_name($jtrans);
		...
		if($opt=='af'){ require_once(MODDIR.'control.php'); // Add or Edit form
			...
			if($jtrans==JT_UMUM){
				...
				$fform->fi('Rek. Kas/Bank',iSuggest('rekkas'));
