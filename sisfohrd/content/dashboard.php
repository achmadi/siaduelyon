...
/*******************************************************/
$tengah .='<div class="panel panel-info">
<div class="panel-heading"><h3 class="panel-title">Reminder Peminjaman</h3></div>
<table class="table">';
$hasil =  $koneksi_db->sql_query( "select p.karyawan,p.pinjaman,max(b.tgl) as tgl,sum(b.bayar) as totalbayar,p.pinjaman-sum(b.bayar) as sisabayar
from hrd_bayar b
inner join hrd_pinjaman p
on p.id=b.pid
group by p.karyawan");
while ($data = $koneksi_db->sql_fetchrow($hasil)) {
$karyawan 	= getnamakaryawan($data['karyawan']);
$pinjaman 	= $data['pinjaman'];
$tgl 	= $data['tgl'];
$totalbayar 	= $data['totalbayar'];
$sisabayar 	= $data['sisabayar'];
$tglbayar 	= datetimes($data['tgl'],False,False);
$tengah .="<tr><td>$karyawan - $tglbayar - Sisa Bayar : Rp.$sisabayar</td></tr>";
}
$tengah .='</table>';
$tengah .='</div>';
$tengah .='</div>';
