...
class xdb{
	...
	function __construct($ta="",$fi="*",$wh="",$od=""){ //table, fields, where, order_by
		$this->table=$ta;
		$this->f=$fi==""?"*":$fi;
		$this->j="";
		$this->w=$wh;
	...
	function getsql(){
		$sql="SELECT ".($this->f==""?"*":$this->f)." FROM ".$this->table.($this->j!=""?" ".$this->j." ":"").($this->w!=""?" WHERE ".$this->w." ":"").($this->g!=""?" GROUP BY ".$this->g." ":"").($this->o!=""?" ORDER BY ".$this->o." ":"").($this->l!=""?" LIMIT ".$this->l." ":"");
		// var_dump($sql);
		return $sql; 
	}
