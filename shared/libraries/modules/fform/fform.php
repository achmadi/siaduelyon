...
class fform{
	...
	function fi($a,$b,$c=''){ // Std Input field (label, field , [field unit])
		$this->rwidths="width:".($this->fwidth-$this->lwidth-18)."px";
		echo '<tr>
				<td width="'.$this->lwidth.'px">'.($a==''?'':$a.':').'</td>
				<td>'.$b.($c==''?'':' &nbsp;'.$c).'</td>
			</tr>';
	}
	...
	function head($t='',$ut=1){
		...
		$out= '<table cellspacing="0" cellpadding="0" width="100%"><tr>
			...
			if($ut==1)
				$out.= '<div id="fform_title'.$this->fformid.'" class="fformtitle" style="'.$this->title_style.'"><span style="margin-left:-2px;float:left">'.$t.'</span><div id="ffload'.$this->fformid.'" class="ffload" style="display:none"></div></div>';
			else 
				$out.= '<div id="fform_title'.$this->fformid.'" class="fformtitle" style="display:none"><span style="margin-left:-2px;float:left">'.$t.'</span><div id="ffload" class="ffload" style="display:none"></div></div>';		
				$out.= '<div id="fformct'.$this->fformid.'" class="fformct" style="width:'.$fw.'px"><div id="fformcb"><div style="">
