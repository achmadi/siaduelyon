...
// iSelect('id_department',$departemen,$dept)); // (tambahan) 
function iSelect($d,$a,$s='',$y='',$cb='',$atr='',$req=''){
	if(is_array($s)) 
		$s=$s[$d];
	$r="<select ".($req==''?'':'required="required"')." 
			class=\"iSelect\" id=\"".$d."\" name=\"".$d."\" ".
			($y==""?"":" style=\"".$y."\" ").
			($cb==""?"":" onchange=\"".$cb."\" ")." 
			onfocus=\"this.className='iSelectx'\" 
			onblur=\"this.className='iSelect'\" ".$atr." 
		>";
	if(is_array($a)){ // jika array 
		if(count($a)>0){ //(data lebih dari 1 record)
			foreach($a as $k=>$v){
...
function iSuggest($id){
	$r = '<input readonly placeholder="kode" class="iText" maxlength="5" size="5" type="text" name="_'.$id.'" id	="_'.$id.'">'; 
	$r .= '<input id="'.$id.'" name="'.$id.'" onkeyup="iSuggestJ(this);" class="iText" size="40" placeholder="rekening" >';
	return $r;
}
...
function inputTanggal($dn,$val="0000-00-00",$cb="",$show="dmY"){
	...
	$tgl =explode("-",$val); 
	$s   =""; 
	if(intval($tgl[1])!=0) // hitung jumlah hari dalam bulan 
