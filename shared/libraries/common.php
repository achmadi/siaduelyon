<?php
	...
	function admin_isoperator(){
		$ADMIN=admin_get();
		if($ADMIN['level']<=ADMIN_LEVEL_OPERATOR) //operator = 2
			return true;
		else 
			return false;
	}
	...
	function notifbox(){
		if(isset($_SESSION[ASID.'notifbox'])){
			if($_SESSION[ASID.'notifbox']!=''){ 
				echo $_SESSION[ASID.'notifbox']; 
				$_SESSION[ASID.'notifbox']='';
			}
	...
	function app_menu_home(){
		global $APP_CSLIDE;
		echo '<li>
				<a href="#&home" onclick="openHome('.$APP_CSLIDE.')" style="border:none" title="Home">
				<img src="'.IMGR.'home.png"/></a>
			</li>';
	}
	...
	function farray(){ // fungsi untuk mendapatkan argumen kemudian memasukkan k dalam array 
		$s=array();
		$a=func_get_args();
		$n=count($a);
		for($i=0;$i<$n;$i++){
